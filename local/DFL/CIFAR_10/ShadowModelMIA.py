...
class AttackModel:
    ...
    def MIA_shadow_model_attack(self):
        ...
        shadow_train_res = torch.load("random_shadow_train_res.pt", map_location=self.device)
        shadow_test_res = torch.load("s_test_results.pt", map_location=self.device)

        shadow_train_pre = shadow_train_res[0].to(self.device)
        shadow_test_pre = shadow_test_res[0].to(self.device)

        in_labels = torch.ones(shadow_train_pre.shape[0], dtype=torch.long, device=self.device)
        out_labels = torch.zeros(shadow_test_pre.shape[0], dtype=torch.long, device=self.device)
        ...
        def in_out_samples_check(model, dataset):
            ...
            with torch.no_grad():
                for batch in dataloader:
                    logits = model(batch.to(self.device))
        ...
        in_eval_pre = torch.load("random_target_train_res.pt", map_location=self.device)
        out_eval_pre = torch.load("test_results.pt", map_location=self.device)
        ...
        cifar10_train_accuracy = self.calculate_accuracy(in_eval_pre[0].to(self.device), in_eval_pre[1].to(self.device))
        cifar10_test_accuracy = self.calculate_accuracy(out_eval_pre[0].to(self.device), out_eval_pre[1].to(self.device))
